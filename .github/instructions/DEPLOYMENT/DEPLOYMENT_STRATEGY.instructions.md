...
0x08005000 ├──────────────────────────────┤
           │ CUSTOM APP FIRMWARE (43 KB)  │  ← Deployed via custom bootloader XMODEM
...
**Steps:**
1. Build custom bootloader with `APP_START_ADDR = 0x08005000` (nested inside app area)
2. Build custom app firmware with vector table at `0x08005000`
3. Flash bootloader-as-app via stock IAP:
   ```powershell
   python tools/flasher/ninebot_flasher.py --port COM3 --target ble --firmware build/ble_bootloader_phase2.bin
   ```
4. Power cycle — stock bootloader jumps to 0x08001000 (custom bootloader)
5. Custom bootloader detects no valid app at 0x08005000 → enters XMODEM mode
6. Flash custom app via XMODEM:
   ```powershell
   python tools/flasher/xmodem_send.py --port COM3 --firmware build/ble_app_phase2.sfw
   ```
...
- [ ] XMODEM receive works (transfers .sfw file)
...
- [ ] Update trigger (button hold) enters XMODEM mode
...
3. The Phase 2 custom bootloader (running from app area) will self-relocate:
   ```powershell
   # From Phase 2 running state, use XMODEM to send the bootloader-flasher image
...
- [ ] XMODEM update path works for future app updates
...
2. Update via STM32 relay: STM32 on BLE board proxies XMODEM to nRF51 UART
...
| 4 | nRF51822 | App (0x00018000) | STM32 relay XMODEM | Via bootloader | Low |
...
3. FLASH    → python tools/flasher/ninebot_flasher.py (stock IAP)
              OR python tools/flasher/xmodem_send.py (custom bootloader)
